...
```
    @[`constexpr`]{.add}@ bool operator<(const unique_ptr<T1, D1>& x, const unique_ptr<T2, D2>& y);       // freestanding
    @[`constexpr`]{.add}@ bool operator>(const unique_ptr<T1, D1>& x, const unique_ptr<T2, D2>& y);       // freestanding
    @[`constexpr`]{.add}@ bool operator<=(const unique_ptr<T1, D1>& x, const unique_ptr<T2, D2>& y);      // freestanding
    @[`constexpr`]{.add}@ bool operator>=(const unique_ptr<T1, D1>& x, const unique_ptr<T2, D2>& y);      // freestanding
    @[`constexpr`]{.add}@ compare_three_way_result_t<typename unique_ptr<T1, D1>::pointer,
    @[`constexpr`]{.add}@ shared_ptr<T> make_shared(Args&&... args);                                  // \tcode{T} is not array
    @[`constexpr`]{.add}@ shared_ptr<T> allocate_shared(const A& a, Args&&... args);                  // \tcode{T} is not array
    @[`constexpr`]{.add}@ shared_ptr<T> make_shared(size_t N);                                        // \tcode{T} is \tcode{U[]}
    @[`constexpr`]{.add}@ shared_ptr<T> allocate_shared(const A& a, size_t N);                        // \tcode{T} is \tcode{U[]}
    @[`constexpr`]{.add}@ shared_ptr<T> make_shared();                                                // \tcode{T} is \tcode{U[N]}
    @[`constexpr`]{.add}@ shared_ptr<T> allocate_shared(const A& a);                                  // \tcode{T} is \tcode{U[N]}
    @[`constexpr`]{.add}@ shared_ptr<T> make_shared(size_t N, const remove_extent_t<T>& u);           // \tcode{T} is \tcode{U[]}
    @[`constexpr`]{.add}@ shared_ptr<T> allocate_shared(const A& a, size_t N,
    @[`constexpr`]{.add}@ shared_ptr<T> make_shared(const remove_extent_t<T>& u);                     // \tcode{T} is \tcode{U[N]}
    @[`constexpr`]{.add}@ shared_ptr<T> allocate_shared(const A& a, const remove_extent_t<T>& u);     // \tcode{T} is \tcode{U[N]}
    @[`constexpr`]{.add}@ shared_ptr<T> make_shared_for_overwrite();                                  // \tcode{T} is not \tcode{U[]}
    @[`constexpr`]{.add}@ shared_ptr<T> allocate_shared_for_overwrite(const A& a);                    // \tcode{T} is not \tcode{U[]}
    @[`constexpr`]{.add}@ shared_ptr<T> make_shared_for_overwrite(size_t N);                          // \tcode{T} is \tcode{U[]}
    @[`constexpr`]{.add}@ shared_ptr<T> allocate_shared_for_overwrite(const A& a, size_t N);          // \tcode{T} is \tcode{U[]}
  ...
  template<class T> @[`constexpr`]{.add}@ void swap(weak_ptr<T>& a, weak_ptr<T>& b) noexcept;
    @[`constexpr`]{.add}@ auto out_ptr(Smart& s, Args&&... args);                                         // freestanding
    @[`constexpr`]{.add}@ auto inout_ptr(Smart& s, Args&&... args);                                       // freestanding
...
template<class U> @[`constexpr`]{.add}@ bool owner_before(const weak_ptr<U>& b) const noexcept;
  ...
  @[`constexpr`]{.add}@ shared_ptr<T> make_shared(Args&&... args);                    // \tcode{T} is not array
  @[`constexpr`]{.add}@ shared_ptr<T> allocate_shared(const A& a, Args&&... args);    // \tcode{T} is not array
  @[`constexpr`]{.add}@ make_shared(size_t N);                                        // \tcode{T} is \tcode{U[]}
  @[`constexpr`]{.add}@ shared_ptr<T> allocate_shared(const A& a, size_t N);          // \tcode{T} is \tcode{U[]}
  @[`constexpr`]{.add}@ shared_ptr<T> make_shared();                                  // \tcode{T} is \tcode{U[N]}
  @[`constexpr`]{.add}@ shared_ptr<T> allocate_shared(const A& a);                    // \tcode{T} is \tcode{U[N]}
  @[`constexpr`]{.add}@ shared_ptr<T> make_shared(size_t N, const remove_extent_t<T>& u);       // \tcode{T} is \tcode{U[]}
  @[`constexpr`]{.add}@ shared_ptr<T> allocate_shared(const A& a, size_t N, const remove_extent_t<T>& u);   // \tcode{T} is \tcode{U[]}
  @[`constexpr`]{.add}@ shared_ptr<T> make_shared(const remove_extent_t<T>& u);       // \tcode{T} is \tcode{U[N]}
  @[`constexpr`]{.add}@ shared_ptr<T> allocate_shared(const A& a, const remove_extent_t<T>& u);   // \tcode{T} is \tcode{U[N]}
