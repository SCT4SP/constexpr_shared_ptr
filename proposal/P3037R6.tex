...
The pointer value returned by a conversion operator, at runtime, holds the
address of a pointer reference member of the relevant smart pointer adaptor
temporary. This member references the pointer managed by the smart pointer.
At runtime, a \texttt{reinterpret\_cast} can be used to obtain a
\texttt{void**} from a \texttt{Pointer*}.
